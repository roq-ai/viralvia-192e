...
import { getDiscountCouponById } from 'apiSdk/discount-coupons';
import { DiscountCouponInterface } from 'interfaces/discount-coupon';

function DiscountCouponViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<DiscountCouponInterface>(
    () => (id ? `/discount-coupons/${id}` : null),
    () =>
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('discount_coupon', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/discount-coupons/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem label="Code" text={data?.code} />
