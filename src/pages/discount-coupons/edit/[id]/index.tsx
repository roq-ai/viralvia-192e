...
import { getDiscountCouponById, updateDiscountCouponById } from 'apiSdk/discount-coupons';
...
function DiscountCouponEditPage() {
  ...
  const handleSubmit = async (values: DiscountCouponInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateDiscountCouponById(id, values);
      mutate(updated);
      resetForm();
