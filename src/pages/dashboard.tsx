...
import { getDiscountCoupons } from 'apiSdk/discount-coupons';
import { DiscountCouponInterface } from 'interfaces/discount-coupon';
import { getCompanyDiscounts } from 'apiSdk/company-discounts';
...
function DashboardPage() {
  ...
  const discountCouponStat = useSWR<PaginatedInterface<DiscountCouponInterface>>(
    () => `/discount-coupons`,
    () => getDiscountCoupons({ limit: 1, offset: 0 }),
  );

  const companyDiscountStat = useSWR<PaginatedInterface<CompanyDiscountInterface>>(
    () => `/company-discounts`,
    () => getCompanyDiscounts({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: discountCouponStat?.data?.totalCount,
      isLoading: discountCouponStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Company Discounts',
      result: companyDiscountStat?.data?.totalCount,
      isLoading: companyDiscountStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Contents',
